...
def train_model(num1, num2, X_profiling, Y_profiling, X_test, Y_test, model, save_file_name, learning_rate, epochs, batch_size):
    check_file_exists(os.path.dirname(save_file_name))
    file_path = save_file_name + 'model_weights_' + str(num1) + '_' + str(num2) + '.h5'
...
def train_model_st(source_data_profiling, source_labels_profiling, target_data_profiling, source_data_test, source_labels_test,
                   model, save_file_name, folder, epochs, batch_size, learning_rate, lambda_mmd, num_classes=2, fepoch=None):
    """Train a CNN with MMD loss for domain adaptation."""
    check_file_exists(os.path.dirname(save_file_name))
    file_path = save_file_name + 'model_weights_mmd2.h5'
    ...
    with tf.device('/GPU:0'):
        ...
        for epoch in range(epochs):
            ...
            # 10-4 -> 0.83333    10-5 -> 0.86667    10-6 -> 0.9    10-7 -> 0.93333
            if val_accuracy >= 0.83333:
                fepoch.write(str(epoch + 1))
                fepoch.write("\n")
                flag = 0
                break
        if flag == 1:
